...
function writeTextToFile(file, text) {
  return window.ipcRenderer.sendSync("write-text-to-file", {
    file: file,
    text: text,
  });
}
  ...
  if (
    ...
    try {
      const obj = JSON.parse(
        new TextDecoder().decode(openAndReadFileResult.data)
      );
      return obj == null ||
        obj.data == null ||
        obj.data.guests == null ||
        obj.data.anniversary == null ||
        obj.data.prizeCount == null ||
        obj.data.excludedGuests == null ||
        obj.data.winners == null
        ? null
        : obj;
    } catch (ignored) {
      return null;
...
function showSaveFileDialogAndExportData(data) {
  ...
  if (
    ...
    return exportDataToFile(showSaveFileDialogResult.data, data);
  }
  return showSaveFileDialogResult;
}
...
function exportDataToFile(file, data) {
  ...
  return writeTextToFile(file, jsonStr);
}

function autoSaveDataToFile(data) {
  return exportDataToFile(
    `autosave_${moment().format("YYYYMMDDHHmmss")}.json`,
    data
  );
}
